...
meravclinic<-simmer("meravclinic") %>%
  add_resource(name="secretary", capacity=1,queue_size=Inf)%>% 
...
secondmemo<-
  ...
  simmer::select(resources = c("tech1","tech2","tech3","tech4"),"first-available", id=5)%>%
...
memo<-
  trajectory("memo check")%>%
  #("i'm in memo check")%>%
  simmer::select(resources = c("tech1","tech2","tech3","tech4"),"first-available", id=1)%>%
...
ultra<-
  ...
  #("i'm in ultra check")%>%
  simmer::select(resources = c("tech1","tech2","tech3","tech4"),"first-available", id=3)%>%
...
memoUltra<-
  ...
  simmer::select(resources = c("tech1","tech2","tech3","tech4"),"first-available", id=7)%>%##choosing the right tech from the attribute
  ...
  #("i'm in ultra check")%>%
  simmer::select(resources = c("tech1","tech2","tech3","tech4"),"first-available", id=8)%>%#choosing the right tech from the attribute
  ...
  simmer::select(resources = c("tech1","tech2","tech3","tech4"),"first-available", id=10)%>%##choosing the right tech from the attribut
...
arrivalDataOp21 <- get_mon_arrivals(simoXn,ongoing=FALSE)
arrivalDataOp22 <- get_mon_arrivals(simoXn,ongoing=TRUE)
arrivalDataOp23 <- get_mon_arrivals(simoXn,ongoing=TRUE,per_resource=TRUE)
resourceDataOp2 <- get_mon_resources(simoXn)
attributeDataOp2 <- get_mon_attributes(simoXn)

##########################################################מדדים#############################################
#1.avereage Q for secretary
QLengthOp2 <- 0
for(i in 1:n ){
  timeOp2 <- as.matrix(subset(resourceDataOp2, resource == "secretary" & replication==i, select = c(time)))
  queueLengthOp2 <- as.matrix(subset(resourceDataOp2, resource == "secretary" & replication==i, select = c(queue)));
  QLengthOp2[i] <-  avgQueue(timeOp2,queueLengthOp2,simulationTime)
}
QLengthOp2 <- as.table(QLengthOp2)
avgQueueLengthForSecOp2 <- mean(QLengthOp2)
stdQLengthOp2 <- sd(QLengthOp2)

paste("the avg Queue length for the secretary is:",avgQueueLengthForSecOp2)

#2.prop of finished

numOfArrivedOp2 <- sqldf("SELECT count(*)
                         FROM arrivalDataOp22
                         where name!='setclinic0' and (name not like 'break%')
                         group by replication")

numOfFinishedOp2 <- sqldf("SELECT count(*) as propration
                          FROM arrivalDataOp21
                          where name!='setclinic0' and (name not like 'break%')
                          group by replication")

propOfFinishedOp2 <- numOfFinishedOp2/numOfArrivedOp2
meanPropOfFinishedOp2 <- sqldf("SELECT avg(propration)
                               From propOfFinishedOp2")

stdPropOfFinishedOp2 <- sqldf("SELECT stdev(propration)
                              From propOfFinishedOp2")


paste("the proporation of customers that finished the day is",meanPropOfFinishedOp2*100,"%")

#3.avereage mean flow

meanFlowByReplicationOp2<-sqldf("select avg(end_time-start_time) as FlowTime
                                from arrivalDataOp21
                                where name!='setclinic0' and (name not like 'break%')
                                group by replication")

meanFlowOp2 <-  sqldf("SELECT avg(FlowTime)
                      From meanFlowByReplicationOp2")
stdMeanFlowOp2 <- sqldf("SELECT stdev(FlowTime)
                        From meanFlowByReplicationOp2")

paste("the mean flow time in system is:",meanFlowOp2, "minutes")
