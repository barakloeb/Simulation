...
meravclinic<-simmer("meravclinic") %>%
  ...
  add_resource(name="memoroom", capacity=2,queue_size=Inf)%>%
...
arrivalData1 <- get_mon_arrivals(simoXn,ongoing=FALSE)
arrivalData2 <- get_mon_arrivals(simoXn,ongoing=TRUE)
arrivalData3 <- get_mon_arrivals(simoXn,ongoing=TRUE,per_resource=TRUE)
resourceData <- get_mon_resources(simoXn)
attributeData <- get_mon_attributes(simoXn)
...
QLength <- 0
for(i in 1:n ){
  time <- as.matrix(subset(resourceData, resource == "secretary" & replication==i, select = c(time)))
  queueLength <- as.matrix(subset(resourceData, resource == "secretary" & replication==i, select = c(queue)));
  QLength[i] <-  avgQueue(time,queueLength,simulationTime)
}
QLength <- as.table(QLength)
avgQueueLengthForSec <- mean(QLength)
stdQLength <- sd(QLength)

paste("the avg Queue length for the secretary is:",avgQueueLengthForSec)

#2.prop of finished
numOfArrived <- sqldf("SELECT count(*)
                        FROM arrivalData2
                        where name!='setclinic0' and (name not like 'break%')
                        group by replication")

numOfFinished <- sqldf("SELECT count(*) as propration
                        FROM arrivalData1
                        where name!='setclinic0' and (name not like 'break%')
                       group by replication")

propOfFinished <- numOfFinished/numOfArrived
meanPropOfFinished <- sqldf("SELECT avg(propration)
                        From propOfFinished")

stdPropOfFinished <- sqldf("SELECT stdev(propration)
                        From propOfFinished")


paste("the proporation of customers that finished the day is",meanPropOfFinished*100,"%")

#3.avereage mean flow

meanFlowByReplication<-sqldf("select avg(end_time-start_time) as FlowTime
             from arrivalData1
             where name!='setclinic0' and (name not like 'break%')
                group by replication")

meanFlow <-  sqldf("SELECT avg(FlowTime)
                        From meanFlowByReplication")
stdMeanFlow <- sqldf("SELECT stdev(FlowTime)
                        From meanFlowByReplication")

paste("the mean flow time in system is:",meanFlow, "minutes")
